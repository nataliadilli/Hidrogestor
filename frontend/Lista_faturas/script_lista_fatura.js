...
// variável global para evitar referência indefinida (pode não ser usada aqui, mas mantém compatibilidade)
...
async function fetchResidentsFromAPI() {
    try {
        ...
        const response = await fetch(`${API_BASE_URL}/listarFaturas/${dadosUsuario.nr_unidadeconsumidora}`);
...
document.getElementById("btnExportarCSV").addEventListener("click", async function () {
    ...
    try {
        ...
        const response = await fetch(`${API_BASE_URL}/exportarFaturas/${id}`, {
            method: "GET"
        });
